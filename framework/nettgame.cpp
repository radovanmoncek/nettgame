...
namespace nettgame { // remove all docs from here, leave only in .hpp
    ...
    template<class GameState>
	void nettgame_server<GameState>::make_multicast_state_call(transferable_game_state game_state) { // make multicast member function to peers
	    unsigned char buffer[1024]; // magic
	    char game_state_buffer[512]; // magic

	    game_state.transferify((unsigned char *)game_state_buffer); // C style cast justified
