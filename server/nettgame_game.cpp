...
int main(int argc, char **argv) {
    ...
    int is_master = std::max<int>(MIN_ASIO_THREADS, atoi(argv[offset++]));
    std::string document_root = argv[offset++];
    boost::asio::io_context io_context{threads};
    nettgame::nettgame_server<game_state> nettgame_server(address_raw, port/*, argv[1], port+1 todo maybe custom load balancer*/, is_master);
