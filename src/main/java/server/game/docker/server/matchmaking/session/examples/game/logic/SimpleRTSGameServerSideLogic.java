...
import server.game.docker.net.MyPDUAction;
...
//                        availableGoldMinesPool2.add(0, new byte[]{i, j});
                    gameSession.sendUnicast(MyPDU.fromWorldInfoData(i, j, TileType.RESOURCENODE.getTileID()).withIPAndPort(currentClientPlayer.getClientIPAddress(), currentClientPlayer.getPort()));
                }
            }
            if(i == gameMap.length - 2){
                //Player side decision and fog of war
                gameSession.sendUnicast(MyPDU.fromWorldInfoData((byte) 0, (byte) 5, Byte.MAX_VALUE).withIPAndPort(findSessionClientWithPlayerClientID(players.get(0).getClientID()).getClientIPAddress(), findSessionClientWithPlayerClientID(players.get(0).getClientID()).getPort()));
                gameSession.sendUnicast(MyPDU.fromWorldInfoData((byte) 10, (byte) 5, Byte.MIN_VALUE).withIPAndPort(findSessionClientWithPlayerClientID(players.get(1).getClientID()).getClientIPAddress(), findSessionClientWithPlayerClientID(players.get(1).getClientID()).getPort()));
            }
        }
    ...
    private void registerActionMappings(){
        gameSession.getActionMapper()
            //Game start
            .withActionMapping(MyPDUTypes.GAMESTART.getID(), p -> {
...
//                                            p.setGold(p.getGold() + 10);
                                ...
                                MyPDU serverGoldTickInfo = new MyPDU(MyPDUTypes.SERVERTICKUPDATE.getID(), p.getGold().toString(), Short.toString((short) (playerMineCount * 10)));
                    ...
                    gameSession.sendMulticast(new MyPDU(MyPDUTypes.GAMESTART.getID()));
                }
            })
            .withActionMapping(MyPDUTypes.PLAYERMOVE.getID(), p -> {
...
//                                currentPlayer.setGold(currentPlayer.getGold() - 1000);
                                ...
                                gameSession.sendMulticast(MyPDU.fromWorldInfoData(boughtMine[0], boughtMine[1], TileType.GOLDMINE.getTileID()));
                        ...
                        case "1" -> {
                            ...
                            if(accessPlayerGoldFunds(currentPlayer) >= 10000){
                                gameSession.sendMulticast(new MyPDU(MyPDUTypes.GAMEEND.getID(), currentPlayer.getClientID().toString(), currentPlayer.getUsername()));
