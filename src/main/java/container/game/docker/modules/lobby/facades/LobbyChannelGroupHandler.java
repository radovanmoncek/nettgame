package container.game.docker.modules.lobby.facades;
...
public class LobbyChannelGroupHandler extends ChannelGroupHandler<LobbyResponseProtocolDataUnit> {
    ...
    public LobbyChannelGroupHandler(PlayerChannelGroupHandler playerServerFacade) {
    ...
    public synchronized void receiveCreateRequest(final Channel clientChannel) {
        ...
        final var lobbyUpdate = new LobbyResponseProtocolDataUnit(
                (byte) LobbyResponseProtocolDataUnit.LobbyUpdateResponseFlag.CREATED.ordinal(),
    ...
    public synchronized void receiveJoinRequest(final Channel clientChannel, final Long lobbyLeaderId) {
        ...
        lobbyMemberships
                ...
                .ifPresent(entry -> {
                    ...
                    final var lobbyUpdateJoined = new LobbyResponseProtocolDataUnit(
                            (byte) LobbyResponseProtocolDataUnit.LobbyUpdateResponseFlag.JOINED.ordinal(),
                            lobbyLeaderId,
                    ...
                    final var lobbyUpdateMemberJoined = new LobbyResponseProtocolDataUnit(
                            (byte) LobbyResponseProtocolDataUnit.LobbyUpdateResponseFlag.MEMBERJOINED.ordinal(),
                            lobbyLeaderId,
                            lobby.stream().map(playerServerFacade::getNickname).toList()
                    );
    ...
    public synchronized void receiveLeaveRequest(final Channel clientChannel) {
        ...
        if (lobby.size() == 2) {
            ...
            final var lobbyUpdateForOtherMembers = new LobbyResponseProtocolDataUnit(
                    (byte) LobbyResponseProtocolDataUnit.LobbyUpdateResponseFlag.MEMBERLEFT.ordinal(),
        ...
        final var lobbyUpdateLeft = new LobbyResponseProtocolDataUnit((byte) LobbyResponseProtocolDataUnit.LobbyUpdateResponseFlag.LEFT.ordinal(), -1L, Collections.emptyList());
